...
func calculate_lead(delta):
	...
	if($Paddle.global_position.distance_to(ball_manager.game_ball.global_position ) < 700):
		...
		var facing_velocity = ball_manager.game_ball.linear_velocity
		var is_facing = facing_velocity.dot($Paddle.transform.basis_xform(Vector2.LEFT)) > 0.0
		if(is_facing):
		#var penality = randf_range(-10,10)	
		
			return target  + (error_diff * 0.12) + d
	return 0
